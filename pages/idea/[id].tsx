...
export default function Idea() {
  const [idea, setIdea] = useState<IdeaList | null>(null);
  const [commentText, setCommentText] = useState<string>("");
  const [commentsList, setCommnetsList] = useState<Comments[]>([]);
  ...
  const getData = async () => {
    const data = await axios({
      url: `${API.basic}/posting/posting/${router.query.id}?orderBy=views`,
      headers: {
  ...
  const getComments = async () => {
    const data = await axios({
      url: `${API.basic}/comments/${router.query.id}`,
      headers: {
        Authrization: localStorage.getItem("token"),
      },
    });
    setCommnetsList(data.data);
  };
  ...
  const clickLike = async () => {
    try {
      await axios.post(
        `${API.basic}/posting/like/${router.query.id}`,
        {},
        {
          headers: {
            Authorization: localStorage.getItem("token"),
      ...
      {idea && (
        <div className="w-[840px]">
          <div className="space-y-5">
            ...
            <p className="text-sm text-gray-400">{idea.category.name}</p>
          ...
          <div className="py-7">
            ...
            <div className="py-10 px-5">
              ...
              <p className="pt-7">{idea.description}</p>
            </div>
          </div>
